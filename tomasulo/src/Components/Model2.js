...
const Controller2 =()=> {
    ...
    let [InstructionFile,setInstructionFile] = useState(new Array());
    let [MulDiv,setMulDiv] = useState(new new Array(3));
    let [AddSub,setAddSub] = useState(new new Array(3));
    let [StoreBuffer,setStoreBuffer] = useState(new Array(3));
    let [LoadBuffer,setLoadBuffer] = useState(new Array(3));
    let [RegisterFile,setRegisterFile] = useState(new Array(20));
    let [MemoryFile,setMemoryFile] = useState(new Array(500));

    let [NumberOfInst,setNumberOfInst] = useState(0);
...
//Instruction File Methods
    const  addInstructionInstructionFile = (inst)=>
    {
        setInstructionFile(InstructionFile.push({instruction : inst,Issue : -1,Execute:-1,ExecuteFinish : -1,Write_Result : -1,index :  NumberOfInst,reservedRomm : ''}));
        setNumberOfInst(NumberOfInst++);
    }
    
    const  getOperationByIndexInstructionFile = (index)=>
    {
        const inst =  instructions[index].instruction.split(/[ ,]+/);
        return inst[0];
    }
    const getOperationByInstInstructionFile =(instruction) =>
    {
        const inst = instruction.instruction.split(/[ ,]+/);
        return inst[0];
    }
    const getDestinationByIndexInstructionFile = (index)=>
    {
        const inst =  instructions[index].instruction.split(/[ ,]+/);
        return inst[1];
    }
    const getDestinationByInstInstructionFile = (instruction)=>
    {
        const inst = instruction.instruction.split(/[ ,]+/);
        return inst[1];
    }
    const getFirstSourceByIndexInstructionFile = (index)=>
    {
        const inst =  instructions[index].instruction.split(/[ ,]+/);
        return inst[2];
    }
    const getFirstSourcenByInstInstructionFile=(instruction)=>
    {
        const inst = instruction.instruction.split(/[ ,]+/);
        return inst[2];
    }
    const getSecondSourceByIndexInstructionFile=(index)=>
    {
        const inst =  instructions[index].instruction.split(/[ ,]+/);
        return inst[3];
    }
    const getSecondSourceByInstInstructionFile=(instruction)=>
    {
        const inst = instruction.instruction.split(/[ ,]+/);
        return inst[3];
    }

    const getAddressInstructionFile=(instruction)=> //LD F2 100    SD F2 0(R6)
    {
        const inst = instruction.instruction.split(/[ ,]+/);
        return inst[2];
    }
 
    const displayInstructionFile=()=>
    {   
        let result = 'Instructions:\n'
       //  result += 'Instruction                   Issue              Execute                Write_Result\n'
        for(let i = 0; i <  instructions.length; i++)
        {
            result += (i+1)+' : '+  displayInstructionInstructionFile( instructions[i].instruction)+'\n';
        }
        return result;
    }
    const displayInstructionInstructionFile=(instruction)=>
    {
        let result = '';
...
const emptyIndexMemoryFile= () => //return index of empty space to reserve for a new inst.
{
    for(let i = 0;i< MemoryFile.length;i++)
    {
        if( MemoryFile[i]== null ||  MemoryFile[i]== undefined ||  MemoryFile[i]== '' ||  MemoryFile[i]== ' ')
        {
            return i;
...
const addWordMemoryFile= (address,content)=>
{

    const word = {Address:address,Content:content};
     MemoryFile.push(word);
}

//getters

const getContentMemoryFile = (address) => //get content of address by the name of the address
{

for(let i = 0;i< MemoryFile.length;i++)
{
    if( MemoryFile[i] == undefined)
        continue;
    if( MemoryFile[i].Address == address)
    {
        return  MemoryFile[i].Content
...
const setContentMemoryFileMemoryFile=(address,content)=>  //set content of address by the name of the address
{ 
    //missing erease qi
    let flag = false;
    for(let i = 0;i< MemoryFile.length;i++)
...
const displayInstructionMemoryFile=(room)=>
{
let result = '';
...
//////////
  ...
  {
    const operation = InstructionFile.InstructionFile(instruction);
    if(operation == 'L.D')
    {
